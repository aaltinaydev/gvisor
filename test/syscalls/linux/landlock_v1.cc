...
TEST(LandlockV1Test, FsconfigReconfigureDeniedWhenDomainActive) {
  SKIP_IF(LandlockAbiVersion() < 1);
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_SYS_ADMIN)));
  // An fsopen(2) descriptor is not save-safe in gVisor: fsconfigfd.Fd is
  // saved with a vfs.GetFilesystemOptions in its context, which the state
  // package cannot encode, so a cooperative save while one is open fails the
  // whole run. That is a pre-existing fsconfigfd issue, not a Landlock one.
  const DisableSave ds;
