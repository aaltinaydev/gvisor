...
TEST(LandlockV1Test, OPathRulesetFdIsEbadf) {
  ...
  EXPECT_THAT(landlock_add_rule(opath_fd, LANDLOCK_RULE_PATH_BENEATH,
                                &path_beneath, 0),
              SyscallFailsWithErrno(EBADF));
  ...
  EXPECT_THAT(landlock_add_rule(plain_fd, LANDLOCK_RULE_PATH_BENEATH,
                                &path_beneath, 0),
              SyscallFailsWithErrno(EBADFD));
...
TEST(LandlockV1Test, OverlayLayerFilesAndMergedFilesAreDifferent) {
  ...
  const Leg legs[] = {
      {merged, merged_fl, kAllowed, "merged rule reaches the merged lower-backed file"},
      {merged, merged_fu, kAllowed, "merged rule reaches the merged upper-backed file"},
...
// Linux selftest: ptrace_test.c scoped_domains.trace, variants parent_domain and nested_domain
...
// Linux selftest: ptrace_test.c scoped_domains.trace, variants sibling_domain and forked_domains
//
...
// Linux selftest: ptrace_test.c scoped_domains.trace, variant child_domain (PTRACE_TRACEME leg)
//
...
// Linux selftest: ptrace_test.c scoped_domains.trace, variant inherited_domain (PTRACE_TRACEME leg)
//
...
// Linux selftest: ptrace_test.c scoped_domains.trace, PTRACE_MODE_READ leg (test_ptrace_read())
//
...
//     reparent_remove, reparent_dom_superset, layout1_bind.reparent_cross_mount,
