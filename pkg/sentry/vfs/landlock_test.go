...
// TestDerivedInodeIdentity verifies the properties the overlay relies on when
// it derives a merged file's identity from a layer file's: the derived
// identity never equals the base, nor any identity of either filesystem made
// directly; two derivations from one base agree and from different bases
// differ; and a derived identity is refused as a base, so a nested overlay
// has to number its files itself.
func TestDerivedInodeIdentity(t *testing.T) {
	layer := &Filesystem{id: 1}
	overlay := &Filesystem{id: 2}
	baseA := MakeInodeIdentity(layer, 7)
	baseB := MakeInodeIdentity(layer, 8)

	dA, ok := MakeDerivedInodeIdentity(overlay, baseA)
	if !ok || !dA.Ok() {
		t.Fatalf("MakeDerivedInodeIdentity(base) = %+v, %v, want an identity", dA, ok)
	}
	if dA2, _ := MakeDerivedInodeIdentity(overlay, baseA); dA2 != dA {
		t.Errorf("two derivations from one base differ: %+v vs %+v", dA, dA2)
	}
	if dB, _ := MakeDerivedInodeIdentity(overlay, baseB); dB == dA {
		t.Errorf("derivations from different bases are equal: %+v", dA)
	}
	if dA == baseA {
		t.Errorf("derived identity equals its base %+v", baseA)
	}
	if own := MakeInodeIdentity(overlay, 7); dA == own {
		t.Errorf("derived identity equals the deriving filesystem's own identity %+v", own)
	}
	if _, ok := MakeDerivedInodeIdentity(overlay, dA); ok {
		t.Errorf("a derived identity was accepted as a base")
	}
	if _, ok := MakeDerivedInodeIdentity(overlay, InodeIdentity{}); ok {
		t.Errorf("the zero identity was accepted as a base")
	}
}
