...
// be on one device of one filesystem.
func id(ino uint64) InodeIdentity {
	return MakeInodeIdentity(testFS, 0 /* devMajor */, 1 /* devMinor */, ino)
}
...
func TestLandlockCheckAccess(t *testing.T) {
	...
	for _, test := range []struct {
		name string
		// layers are the rulesets to merge, outermost first.
...
// the same device numbers and inode number.
//
// GetAnonBlockDevMinor hands a minor back to the pool when its filesystem is
// destroyed, and rewinds the allocator so that minor is handed out next. A rule
// keyed on a file of the destroyed filesystem would otherwise start matching an
// unrelated file on whichever filesystem was created next, granting access the
// domain never allowed.
func TestInodeIdentityIsScopedToFilesystem(t *testing.T) {
	// Same device numbers and same inode number, different filesystems, as
	// happens when a filesystem is destroyed and its device minor is reused.
	...
	// to the file that inherited its device number.
	const read = linux.LANDLOCK_ACCESS_FS_READ_FILE
