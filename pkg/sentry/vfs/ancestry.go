...
// ino on the device numbered (devMajor, devMinor).
//
...
// is not a valid ino.
//
// The identity is scoped to fs because device numbers are recycled: an
...
// Without fs, an identity held from the destroyed filesystem could come to
// match a file on the new one. fs identifies the filesystem for as long as the
// sentry runs, so identities from distinct filesystems never collide even if
// their device numbers do.
