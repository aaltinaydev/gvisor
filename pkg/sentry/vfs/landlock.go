...
type LandlockRuleset struct {
	...
	// fs.inoByKey,
	// which is never pruned and so reports a single inode number for both files
	// indefinitely; a rule differs in what the confusion costs rather than in how
	// long it lasts, since a stale entry there only yields a misleading st_ino
	// while a stale rule grants access.
...
func (rp *ResolvingPath) CheckLandlockOpenCreate(ctx context.Context, parent *Dentry, opts *OpenOptions) error {
	...
	accessRights := linux.LANDLOCK_ACCESS_FS_MAKE_REG | landlockOpenAccessRights(opts, false /* isDir */)
...
// calls security_sb_mount(), SYSCALL_DEFINE5(move_mount, ...) resolves the
// destination and then the source before vfs_move_mount() calls
// security_move_mount(), and SYSCALL_DEFINE2(pivot_root, ...) resolves both
// directories before path_pivot_root() calls security_sb_pivotroot().
