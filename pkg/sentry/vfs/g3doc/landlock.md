...
(`pkg/sentry/vfs/pathname.go`), a new VFS primitive: each `FilesystemImpl`
...
- **overlay**: identity must survive **copy-up**, or a rule added on a
  ...
  post-copy-up dentry still produces the original key; the key is then
  translated, through a per-overlay table, into an inode number scoped to the
  overlay filesystem itself. Pinning an
  ...
  ```
    ...
    (rule keyed on O(L), the overlay's           identityOrigins[U] → L → O(L)
     own number for key L; L itself           → the rule still matches
