...
in v1: `TRUNCATE` (v3), `IOCTL_DEV` (v5), `RESOLVE_UNIX` (v9), network rules
(v4), and IPC scoping (v6) — under v1 those operations are simply unrestricted.
...
- **overlay**: identity must survive **copy-up**, or a rule added on a
  ...
  yet it must not *be* the layer file's identity: on Linux the overlay inode is
  distinct from the lower and upper inodes, so a rule on a merged directory
  grants nothing on the layer directory it is built from and vice versa (the
  selftest `layout2_overlay.same_content_different_file` checks both
  directions). Overlay therefore derives a *key* from the topmost *lower*
  layer on which the file exists when there is one, and for pinned lower
  identities records an `upper → origin` mapping at copy-up time so the
  post-copy-up dentry still produces the original key; the overlay's identity
  is then *derived* from that key by `vfs.MakeDerivedInodeIdentity`, which
  scopes it to the overlay by recording the key's filesystem alongside its
  inode number, with no per-file bookkeeping (a table is used only for a
  layer that is itself an overlay). Pinning an
  ...
  ```
    ...
    └─────────┘                               └─────────┘
    InodeIdentity() = O(L)                    InodeIdentity() =
    (rule keyed on O(L), the identity the        identityOrigins[U] → L → O(L)
     overlay derives from L; L itself         → the rule still matches
     never matches a rule on O(L))
  ```
...
- In gofer's `unlinkAt`, the child dentry is force-loaded when Landlock is in
  ...
  cost at zero for non-Landlock workloads). The required right itself comes
  from the syscall, `REMOVE_FILE` for `unlink(2)` and `REMOVE_DIR` for
  `rmdir(2)`, exactly as Linux's `hook_path_unlink()` and `hook_path_rmdir()`
  do; the victim is loaded so that its inode number can be retired on
  deletion, and so that a missing victim reports `ENOENT` before the hook's
  `EACCES`, in `do_unlinkat()`'s order.
...
- `test/syscalls/linux/landlock_v1_selftests.cc` (100 tests ported from the
  kernel's `tools/testing/selftests/landlock`, each annotated with its
  upstream fixture and test, closing with the list of selftests not ported and
  why) and `test/syscalls/linux/landlock_v1.cc` (52 gVisor-originated tests:
  VFS error ordering, host-donated FDs, mqueue, fsconfig, overlay specifics,
  and symlink races), sharing `landlock_util.h`: together a conformance suite
  covering the syscall ABI edge cases, every v1 access right, layer stacking,
  rename/link `EXDEV`/`EACCES` priority, bind-mount and disconnected-directory
  ancestor walks, ptrace scoping, and exec inheritance. The suite passes
