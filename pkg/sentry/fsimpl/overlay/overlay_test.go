...
func TestPinnedInodeIdentitySurvivesCopyUp(t *testing.T) {
	for _, test := range []struct {
		name string
		// pinAfterCopyUp pins the identity after the file has been copied up
		// rather than before, using the dentry that predates the copy-up.
		pinAfterCopyUp bool
	}{
...
func TestPinnedInodeIdentitySurvivesDirectoryRename(t *testing.T) {
	...
	for _, test := range []struct {
		name string
		// path is the file whose identity the test pins, and renamedPath is
		// where that file lives after the rename.
		path        string
		renamedPath string
	}{
