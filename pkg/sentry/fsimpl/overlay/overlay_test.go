...
func TestPinInodeIdentityPropagatesToLayers(t *testing.T) {
	...
	// The stubs borrow the lower layer's filesystem for their identities: an
	// identity needs a filesystem with an ID, which only a live Filesystem has.
	layerFS := o.lowerRoot.Mount().Filesystem()
	newLayer := func(ino uint64) *pinRecordingDentry {
		l := &pinRecordingDentry{identity: vfs.MakeInodeIdentity(layerFS, ino)}
		l.vfsd.Init(l)
		return l
	}
