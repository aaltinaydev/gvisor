...
// bookkeeping only across a copy-up, which replaces the file the identity comes
// from with a copy in another filesystem, so only files with a lower layer cost
// anything, and only those a caller has pinned.
