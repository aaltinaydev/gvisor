...
ID, device major/minor, inode number)`. The filesystem ID is a per-
`vfs.Filesystem`-instance counter, which prevents two different filesystem
instances that happen to expose the same `(dev, ino)` pair from aliasing each
other's rules. Each `DentryImpl` must produce its identity; implementations
...
- **overlay**: identity must survive **copy-up**, or a rule added on a
  ...
  Linux, where overlayfs exposes a stable inode across copy-up.
